...
export function GoogleLoginButton() {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="group relative flex h-14 w-full items-center justify-center gap-3 overflow-hidden rounded-2xl text-sm font-semibold tracking-wide transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed"
      style={{
    ...
    >
      ...
      {loading ? (
        <>
          <svg className="size-4 animate-spin text-[#1a1208]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <span className="text-[#1a1208]">Redirecting…</span>
        </>
      ) : (
        <>
          {/* Google G mark */}
          ...
          <span className="text-[#1a1208]">Continue with Google</span>
        </>
      )}
    </button>
  )
