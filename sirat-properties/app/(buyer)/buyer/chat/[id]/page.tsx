...
export default async function BuyerChatPage({
  ...
  const { data: chat } = await supabase
    ...
    .select(`
      ...
      properties(title),
      agent:users!agent_id(id, profiles(full_name))
    `)
    .eq('id', id)
    .eq('buyer_id', userId)
    .single()
  ...
  const agentName = (chat.agent as any)?.profiles?.full_name ?? 'Agent'
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      <DashboardPageHeader
        eyebrow="Buyer chat"
        title={agentName}
        description={`Conversation about ${(chat.properties as any)?.title ?? 'your selected property'}.`}
        action={
          <Link
            href="/buyer/chat"
      ...
      <ChatThread chatId={id} currentUserId={userId} otherUserName={agentName} initialMessages={messages ?? []} />
    </div>
  )
}
